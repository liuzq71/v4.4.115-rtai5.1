...
static inline int rtModifyRParam(int i, int j, double *param)
{
  if (j >= 0 && j < lenRPAR[i]) {
    set_double(&RPAR[i][j], param);
/*     RPAR[i][j] = *param; */
    if (verbose) {
      printf("rpar_%d[%d] : %le.\n", i,j, RPAR[i][j]);
    }
...
static inline int rtModifyIParam(int i, int j, int param)
{
  if (j >= 0 && j < lenIPAR[i]) {
    IPAR[i][j] = param;
    if (verbose) {
